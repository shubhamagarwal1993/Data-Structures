...
unsigned long Game::make_match(const string& party_id)
{
    ...
    if(temp != parties.end())
    {
        for(auto& mem : (temp->second).members)
        {
...
unsigned long Game::find_match(const string& party_id, long tol)
{
    ...
    if(temp != parties.end())
    {
    	...
    	long temp_count, match_count, average, my_matches = 0;
//        long temp_count = 0;
//        long match_count = 0;
//        long average = 0;
//        long my_matches = 0;
       ...
       for(auto& mem : (temp->second).members)
       {
            average = average + ((users.find(mem))->second).us.experience;
            temp_count++;
       }
		...
		for(auto& mat : matches)
		{
            ...
            {
                my_matches = my_matches + ((users.find(t1))->second).us.experience;
                center = ((users.find(t1))->second).match_id;
                match_count++;
            }
            
            for(auto& t2 : mat.second.team2)
            {
                my_matches = my_matches + ((users.find(t2))->second).us.experience;
                center = ((users.find(t2))->second).match_id;
                match_count++;
            }
            
            if(match_count)
                my_matches = my_matches/match_count;
            
            if(std::abs(average - my_matches) <= tol)
                return center;
...
string Game::make_party(const vector< string >& usernames)
{
    ...
    for(auto& loop : usernames)
    {
        ...
        {
            name = name + loop;
            if(((person->second).party_id != "") || ((person->second).status != User::ONLINE))
                return "";
        }
        else return "";
    }
    for(auto& loop : usernames)
    {
        (users.find(loop))->second.party_id = name;
    }
    Party throw_my_party;
    throw_my_party.members = usernames;
 ...
 */
int Game::remove_party_from_match(const std::string& party_id, unsigned long match_id)
{
    auto match_itr = matches.find(match_id);
    if(match_itr != matches.end())
    {
 ...
 */
int Game::end_match(unsigned long match_id, long team1_xp, long team2_xp, unsigned long elapsed_time)
{
    auto match_itr = matches.find(match_id);
    if(match_itr != matches.end() && match_itr->second.status == Match::IN_GAME)
    {
        ...
        end_match_update(match_itr->second.team1, processed_parties, team1_xp, elapsed_time, team1_won);
        end_match_update(match_itr->second.team2, processed_parties, team2_xp, elapsed_time, ! team1_won);
